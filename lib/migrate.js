...
function validateOptions (options) {
  const schema = Joi.object().keys({
    ...
    allowTs: Joi.boolean().default(0)
      .description('Allow typescript migration files. Rethinkdb-migrate must be launched using ts-node for this to work'),
...
function getMigrationsFromPath (options) {
  const { migrationsDirectory, relativeTo, allowTs } = options
  console.log('getMigrationsFromPath allowTs', allowTs);
  const path = Path.resolve(relativeTo, migrationsDirectory)
  const migrationRegExp = allowTs ? /^(\d{14})-(.*)\.(js|ts)$/ :  /^(\d{14})-(.*)\.js$/
